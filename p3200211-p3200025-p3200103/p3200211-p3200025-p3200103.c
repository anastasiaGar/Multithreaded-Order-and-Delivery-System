...
void *gabagool(void *order_id){
    ...
    if (flag==1) {
        ...
        order_time = ((order_prep.tv_sec - order_start.tv_sec) + (order_bake.tv_sec - order_prep.tv_sec) +
        (order_baked.tv_sec - order_bake.tv_sec) + (order_packed.tv_sec - order_pack_start.tv_sec));
        ...
        sleep(deliverytime);
        clock_gettime(CLOCK_REALTIME, &order_delivered); // Update order_packed after packing
